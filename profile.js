...
async function loadProfileData(profileId) {
    try {
        ...
        // Get likes count instead of post count
        const { count: likes, error: likesError } = await supabase
            .from('likes')
            .select('*', { count: 'exact', head: true })
            .eq('user_id', profileId);
        
        const totalLikes = likesError ? 0 : likes;
        ...
        // Display the profile with likes instead of posts
        displayProfile(profile, followers, following, totalLikes);
...
function displayProfile(profile, followers, following, likes) {
    ...
    // Set stats (now showing likes instead of posts)
...
function setupEventListeners() {
    ...
    shareProfileButton.addEventListener('click', async () => {
        try {
            const profileUrl = `${window.location.origin}/profile.html?user=${profileData.username}`;
            
            if (navigator.share) {
                // Use Web Share API if available
                await navigator.share({
                    title: `Check out ${profileData.username}'s profile`,
                    text: `View ${profileData.username}'s profile on our app`,
                    url: profileUrl
                });
            } else {
